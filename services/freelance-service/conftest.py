...
def comment(post, mock_user):
    ...
    return Comment.objects.create(
        post=post, author_id=mock_user.id, content="Test comment content"
    )
...
def channel_membership(channel, mock_user):
    ...
    return ChannelMembership.objects.create(
        channel=channel, user_id=mock_user.id, role=ChannelRole.MEMBER
    )
...
def owner_membership(channel, mock_user):
    ...
    return ChannelMembership.objects.create(
        channel=channel, user_id=mock_user.id, role=ChannelRole.OWNER
    )
...
def admin_membership(channel):
    ...
    return ChannelMembership.objects.create(
        channel=channel, user_id=2, role=ChannelRole.ADMIN
    )
...
def sample_channel_data():
    ...
    return {"name": "New Channel", "description": "New channel description"}
...
def sample_comment_data():
    ...
    return {"content": "New comment content"}
